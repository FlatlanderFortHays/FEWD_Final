...
function createSelectOptions(users) {
    if (!users || users === {}) return undefined;
    const options = [];
...
// i. You may want to define an empty toggleComments function for now. The listener test will NOT pass for addButtonListeners until toggleComments is completed. Nevertheless, I recommend waiting
...
async function getUserPosts(userId) {
    if (!userId) {
        console.error("A valid userId is required");
        return undefined;
    }
...
async function getPostComments(postId) {
    if (!postId) {
        console.error("A valid postId is required");
        return undefined;
    }
...
async function createPosts(posts) {
    ...
    for (const post of posts) {
        ...
        const catchPhraseParagraph = createElemWithText(
            "p",
...
async function displayPosts(posts) {
    ...
    const element = posts && posts.length ? await createPosts(posts) : createElemWithText("p", "Select an Employee to display their posts.", "default-text");
...
function toggleComments(event, postId) {
    if (!event || !postId) {
        console.error("Event and postId are required");
        return undefined;
    }
